...
export default class PromptGenerator {
	...
	private async collectFilesWithBinaryInfo(root: FileTreeNode) {
		const files = FileTreeNode.GetAllFilesNodes(root);

		//

		const isBinaryMap = await (async () => {
			const arr = await Promise.all(
				files.map(file => Utils.IsFileBinary(file.absPath))
			);

			const map = new WeakMap<FileTreeNode, boolean>();
			files.forEach((file, index) => map.set(file, arr[index]));
