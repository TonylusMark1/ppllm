...
export default function GetCLIOptions(): CLIOptions {
    ...
    cmd
        ...
        .option('-s, --store', 'Store used settings into root config', false)
    ...
    if (cli.binary) {
        if (!['none', 'tree', 'all'].includes(cli.binary)) {
            console.error(`${cli.emoji ? `${Consts.EMOJI.error} ` : ''}Invalid value for --binary: ${cli.binary}`);
            process.exit(1);
        }
    }

    if (!['file', 'stdout'].includes(cli.output)) {
        console.error(`${cli.emoji ? `${Consts.EMOJI.error} ` : ''}Invalid value for --output: ${cli.output}`);
        process.exit(1);
    }
